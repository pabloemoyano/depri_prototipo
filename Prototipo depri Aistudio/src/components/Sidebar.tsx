...
export const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  const [expandedMenus, setExpandedMenus] = useState<Record<string, boolean>>(() => {
    try {
      ...
      return saved ? JSON.parse(saved) : { reports: true, financial: true };
    } catch {
      return { reports: true, financial: true };
    }
  });
  ...
  const menuItems = [
    ...
    { id: "sales", label: "Ventas / TPV", icon: ShoppingCart },
                      ...
                      {item.subItems.map((sub: any) => {
                        ...
                        return (
                          <button
                            ...
                            className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-[11px] font-bold tracking-wide transition leading-none select-none ${
                              subActive
